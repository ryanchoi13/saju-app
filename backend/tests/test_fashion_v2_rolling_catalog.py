...
def test_owner_review_page_is_noindex_and_uses_sixteen_calibration_boards():
    ...
    boards = re.findall(r"file:'(sample-v(?:[4-9]|1[015])-[^']+\.webp)'", html)
    ...
    assert sum(name.startswith("sample-v6-") for name in boards) == 0
    ...
    assert sum(name.startswith("sample-v15-") for name in boards) == 1
    ...
    assert "일부 승인 화보는 운영 추천에도 사용됩니다" in html
