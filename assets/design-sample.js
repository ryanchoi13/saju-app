...
(() => {
  ...
  window.startDalhaDesignSample = () => {
    ...
    const palette={tpo:'casual',gender:'female',style_mood:'casual',mood_tag:'캐주얼',top:color('아이보리','#F2EBDC'),bottom:color('네이비','#263C55'),mood_desc:'지우님을 위해 두 가지 코디를 제안드리니, 오늘의 코디에 참고해 보세요.',looks:[
      {title:'차분한 데님 코디',items:[item('니트','아이보리','#F2EBDC',3),item('데님 팬츠','블루','#466989',8),item('스니커즈','화이트','#F5F5F0',12)]},
      {title:'편안한 셔츠 코디',items:[item('셔츠','스카이블루','#B9CDDC',2),item('치노 팬츠','베이지','#C8B696',7),item('로퍼','브라운','#745442',13)]}
    ]};
    const business={...palette,tpo:'smart_casual',style_mood:'smart_casual',mood_tag:'비즈니스 캐주얼',looks:[{title:'단정한 셔츠와 슬랙스',items:[item('셔츠','화이트','#F5F4EE',2),item('슬랙스','네이비','#263C55',7),item('로퍼','브라운','#745442',13)]},{title:'차분한 니트와 팬츠',items:[item('니트','아이보리','#F2EBDC',3),item('슬랙스','차콜','#454B50',7),item('로퍼','블랙','#272C31',13)]}]};
