...
Reviewed boards are now rolled out as pre-generated static WebP assets rather than assembled in the browser. The live payload may supply an owner-approved board directly when gender, age band, TPO, role, and weather family all match a reviewed complete-look pair. Existing Wada-colored boards still require an exact template ID and applied color-name sequence. Unmatched scopes keep the text recommendation and never reuse a misleading image.

The production recommendation builder now receives international age. Women from their thirties onward receive one TPO-appropriate bag in both Daily and Trendy looks. Every fifty-plus look, for both genders and all three TPOs, must pass the comfort-footwear gate before color application. The reviewed male fifty-plus cool-weather formal Daily board uses the v11 full-length suit layout with the trouser waist connected beneath the jacket.
