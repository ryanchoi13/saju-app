...
AUTHENTIC_TALISMAN_MATRIX = {
    ...
    "earth": {
        ...
        "career": {"title": "태산 안착부 (泰山 安着符)", "power": "취업 성공 · 안정적 정착 · 계약 체결", "desc": "태산처럼 흔들리지 않는 기반을 마련하여 원하는 직장이나 프로젝트에 안착하게 돕는 부적입니다.", "talisman_type": "earth_career", "seal_text": "安着"},
...
def get_zodiac_fortune(type: str, key: str):
    ...
    if type == "star":
        ...
        # [정통 명리학 기반 12간지 역학 엔진]
        gan_list = ["甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"]
        ji_list = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"]
        
        # 1. 오늘의 일진 간지(日干, 日支) 산출
        base_date = datetime.date(1900, 1, 1)
        diff_days = (today - base_date).days
        today_gan = gan_list[diff_days % 10]
        today_ji = ji_list[(diff_days + 10) % 12]
        
        ji_names = ["쥐", "소", "호랑이", "토끼", "용", "뱀", "말", "양", "원숭이", "닭", "개", "돼지"]
        target_ji = ji_list[ji_names.index(key)] if key in ji_names else "子"

        # 2. 지지 간 합·충·형·상생 역학 분석
        ...
        yuk_chung = {("子","午"),("丑","未"),("寅","申"),("卯","酉"),("辰","戌"),("巳","亥")}
        
        is_yuk_hap = (today_ji, target_ji) in yuk_hap or (target_ji, today_ji) in yuk_hap
        is_sam_hap = any((today_ji in g and target_ji in g) for g in sam_hap)
        is_chung = (today_ji, target_ji) in yuk_chung or (target_ji, today_ji) in yuk_chung

        ji_elem = {
            "子":"water", "丑":"earth", "寅":"wood", "卯":"wood", "辰":"earth", "巳":"fire",
            "午":"fire", "未":"earth", "申":"metal", "酉":"metal", "戌":"earth", "亥":"water"
        }
        t_elem = ji_elem.get(today_ji, "wood")
        my_elem = ji_elem.get(target_ji, "wood")
        
        elem_rel = {
            ("water","wood"): "생(生)", ("wood","fire"): "생(生)", ("fire","earth"): "생(生)", ("earth","metal"): "생(生)", ("metal","water"): "생(生)",
        ...
        zodiac_years_map = {
            "쥐": [("2008", "戊子"), ("1996", "丙子"), ("1984", "甲子"), ("1972", "壬子"), ("1960", "庚子")],
            "소": [("2009", "己丑"), ("1997", "丁丑"), ("1985", "乙丑"), ("1973", "癸丑"), ("1961", "辛丑")],
            "호랑이": [("2010", "庚寅"), ("1998", "戊寅"), ("1986", "丙寅"), ("1974", "甲寅"), ("1962", "壬寅")],
            "토끼": [("2011", "辛卯"), ("1999", "己卯"), ("1987", "丁卯"), ("1975", "乙卯"), ("1963", "癸卯")],
            "용": [("2000", "庚辰"), ("1988", "戊辰"), ("1976", "丙辰"), ("1964", "甲辰"), ("1952", "壬辰")],
            "뱀": [("2001", "辛巳"), ("1989", "己巳"), ("1977", "丁巳"), ("1965", "乙巳"), ("1953", "癸巳")],
            "말": [("2002", "壬午"), ("1990", "庚午"), ("1978", "戊午"), ("1966", "丙午"), ("1954", "甲午")],
            "양": [("2003", "癸未"), ("1991", "辛未"), ("1979", "己未"), ("1967", "丁未"), ("1955", "乙未")],
            "원숭이": [("2004", "甲申"), ("1992", "壬申"), ("1980", "庚申"), ("1968", "戊申"), ("1956", "丙申")],
            "닭": [("2005", "乙酉"), ("1993", "癸酉"), ("1981", "辛酉"), ("1969", "己酉"), ("1957", "丁酉")],
            "개": [("2006", "丙戌"), ("1994", "甲戌"), ("1982", "壬戌"), ("1970", "庚戌"), ("1958", "戊戌")],
            "돼지": [("2007", "丁亥"), ("1995", "乙亥"), ("1983", "癸亥"), ("1971", "辛亥"), ("1959", "己亥")]
        }

        gan_elem = {"甲":"wood","乙":"wood","丙":"fire","丁":"fire","戊":"earth","己":"earth","庚":"metal","辛":"metal","壬":"water","癸":"water"}
        sipsin_guides = {
            "비겁": ["동료들과의 협업이 큰 힘이 되는 날입니다.", "팀워크를 발휘할 때 더 큰 시너지가 납니다.", "주변 사람들과의 성과 나눔이 길운을 부릅니다."],
            "식상": ["번뜩이는 아이디어가 빛을 발하는 날입니다.", "새로운 기획이나 제안을 적극 개진하세요.", "창의적인 실행력이 주변의 큰 호응을 얻습니다."],
            "재성": ["재물 흐름이 원활해지고 실속을 챙깁니다.", "투자나 자산 관리에서 유리한 기회가 옵니다.", "충동지출만 피하면 만족스러운 결실을 봅니다."],
            "관성": ["조직과 직장에서 책임과 명예가 오릅니다.", "결단력 있는 태도로 주변의 신뢰를 독차지합니다.", "원칙과 약속을 준수할 때 승진과 계약이 따릅니다."],
            "인성": ["학업과 자격, 서류 검토에 가장 길한 날입니다.", "선배나 전문가의 조언 속에 명쾌한 해답이 있습니다.", "차분한 마인드 컨트롤이 오랜 고민을 해결합니다."]
        }

        def get_sipsin(t_gan, y_gan):
            te = gan_elem.get(t_gan, "wood")
            ye = gan_elem.get(y_gan, "wood")
            if te == ye: return "비겁"
            elem_cycle = ["wood", "fire", "earth", "metal", "water"]
            t_idx, y_idx = elem_cycle.index(te), elem_cycle.index(ye)
            diff = (t_idx - y_idx) % 5
            if diff == 1: return "식상"
            if diff == 2: return "재성"
            if diff == 3: return "관성"
        ...
        years_data = zodiac_years_map.get(key, zodiac_years_map["말"])
        tips = []
        for y_str, y_ganji in years_data:
            age = 2026 - int(y_str) + 1
            s_type = get_sipsin(today_gan, y_ganji[0])
            tip_str = rng.choice(sipsin_guides[s_type])
            tips.append({
                "year_label": f"{y_str}년생 ({y_ganji} · 만 {age}세)",
                "tip": f"[{s_type}운] {tip_str}"
            })

        times_pool = ["오전 09시 ~ 11시", "오전 11시 ~ 오후 01시", "오후 01시 ~ 03시", "오후 03시 ~ 05시", "오후 05시 ~ 07시"]
        matches = {"쥐":"소띠·용띠", "소":"쥐띠·뱀띠", "호랑이":"말띠·개띠", "토끼":"양띠·돼지띠", "용":"쥐띠·원숭이띠", "뱀":"소띠·닭띠", "말":"호랑이띠·양띠", "양":"토끼띠·말띠", "원숭이":"쥐띠·용띠", "닭":"소띠·뱀띠", "개":"호랑이띠·말띠", "돼지":"토끼띠·양띠"}

        return {
            "name": f"{key}띠",
            "score": score,
            "title": f"{today.strftime('%m월 %d일')} ({today_gan}{today_ji}일) {key}띠 정밀 일진",
            "overview": overview,
            "lucky_time": rng.choice(times_pool),
            "lucky_match": f"찰떡궁합: {matches.get(key, '합이 맞는 인연')}",
            "year_tips": tips
        }
