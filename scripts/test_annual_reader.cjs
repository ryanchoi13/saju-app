...
(async()=>{
    ...
    assert.doesNotMatch(printed.window.document.head.textContent,/break-before:page|page-break-before:always/);
    assert.match(printed.window.document.head.textContent,/margin-top:60pt/);
    assert.doesNotMatch(printed.window.document.body.textContent,/생활 조언으로 표시한 분야에는/);
    printed.window.close();
    assert.equal(doc.getElementById('readingReportMeta').textContent,'결제일 · 2026.02.01');
    assert.equal(doc.getElementById('archiveModalTitle').firstChild.textContent,'올해·월별 운세');
    assert.equal(doc.querySelector('#archiveModalTitle .reading-report-year').textContent,'2026년');
