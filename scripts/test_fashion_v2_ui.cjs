...
assert.equal(w.document.querySelector('#fashionV2DailySlide .fashion-v2-look-summary').textContent,'베이지 블루종과 아이보리 긴팔 티셔츠, 데님 블루 스트레이트 청바지, 그레이 레트로 운동화를 매치해 보세요.');
...
assert.equal(vm.runInContext('fashionV2Narrative('+JSON.stringify(warmDaily)+')',dom.getInternalVMContext()),'라이트 카멜 반팔 폴로와 그레이 경량 스트레이트 팬츠, 다크 브라운 가죽 운동화를 매치해 보세요. 아침저녁에는 쌀쌀할 수 있으니 네이비 얇은 바람막이를 챙기면 좋습니다.');
