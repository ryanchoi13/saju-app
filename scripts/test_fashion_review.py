...
def main():
    ...
    files = re.findall(r"file:'(sample-v[456789]-[^']+\.webp)'", html)
    ...
    assert sum(filename.startswith("sample-v6-") for filename in files) == 2
    ...
    assert sum(filename.startswith("sample-v9-") for filename in files) == 6
