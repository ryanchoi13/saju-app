...
(async () => {
    ...
    await test('home and every pre-existing DOM target remain available', () => {
        ...
        for (const id of ['outfitCards','openOutfitModalButton','menuPhotoCards','wardrobeStorageNotice','menuRecommendationComment','menuExplorerControls','menuError']) newHome.getElementById(id).remove();
